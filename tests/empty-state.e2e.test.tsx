...
test("A5Y: zero recommendation data renders the designed honest empty state", () => {
  const html = renderToStaticMarkup(<RecommendationList outcome={{ kind: "nothing-fits", items: [], message: "No sourced fit." }} query={{ hardware: { platform: "cpu", availableMemoryGb: 32 }, task: "general", desiredContextK: 16, strategy: "balanced" }} selectedId={null} onSelect={() => undefined} />);
