...
| `runner-import-bundle` | M-O can compose the existing v1 handoff with the M-E receipt already carried beside the candidate | M-J already consumes the validated receipt separately from candidate/inventory facts | direct composition | The additive bundle preserves the nested handoff without changing it, canonically hashes the complete two-payload snapshot, and binds candidate, artifact hash, runtime, OS, backend and quantization across both objects. Import validation is side-effect free; expiry and permission behavior remain M-O policy. |
