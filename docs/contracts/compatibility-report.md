...
|---|---|---|---|
| Envelope `schemaVersion`, 12-value `contract`, `status`, typed `data`, `provenance`; partial `completeness.{complete,missing,warnings,recoverableActions}`; non-data `reasonCode,message,recoverableActions,warnings` | No shared envelope. `RecommendationOutcome.kind` is domain-specific. | Commands return structs or `Result<_, String>`. | **Missing/shared-boundary break.** M-A supplies DTOs/reason codes. M-O later maps domain unions/free-form errors. Partial data may never be an arbitrary object. |
