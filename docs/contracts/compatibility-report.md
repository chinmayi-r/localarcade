...
|---|---|---|---|
| Envelope `schemaVersion`, 13-value `contract`, `status`, typed `data`, `provenance`; partial `completeness.{complete,missing,warnings,recoverableActions}`; non-data `reasonCode,message,recoverableActions,warnings` | No legacy shared envelope. `RecommendationOutcome.kind` is domain-specific. | Commands return structs or `Result<_, String>`. | **Shared boundary implemented.** M-A supplies DTOs/reason codes. M-O later maps domain unions/free-form errors. Partial data may never be an arbitrary object. |
