...
| M-O finder→M-H→detail→v1 handoff→U28 bundle | 16 synthetic behavior tests plus purity proof | typed runner execution transport exists separately; no finder frontend wiring | none | boundary/partial | U27 capture can produce proposed-unreviewed evidence, but no reviewed real profile/policy admits M-F data and no production finder consumer joins the paths | [T16][T24][T25] |
