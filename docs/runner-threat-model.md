...
- M7: update metadata design follows The Update Framework's threat model — freshness, rollback protection, key compromise recovery (https://theupdateframework.io/docs/security/). **OPEN:** choosing the concrete TUF-conformant implementation (e.g. platform updater vs. tuf-js/go-tuf) is an unresolved hard-to-reverse decision — stop-and-ask before building the updater.
