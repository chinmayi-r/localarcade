...
| U27 | Which concrete M-F capacity reserves/margin and profile-collection protocol are approved for production admission? | The pure admission mechanism and a synthetic raw-collection/derivation/policy-sweep harness are proven. The harness rejects caller-asserted `local-measurement` because no trusted capture boundary exists, and its production-readiness proof is intentionally unavailable. No production trust manifest exists; the checked Qwen source omits complete runtime/hardware scope and legacy reserve/margin constants have no approved M-A policy source. | Approve a trusted capture protocol plus one scoped policy; collect and independently review new records; keep M-F unavailable in production. | Define capture/attestation, repetition and adverse-load rules; evaluate false-fit/no-fit behavior; then approve one concrete policy. Do not promote synthetic fixtures, request-supplied trust, or legacy defaults. | Real M-F producer and M-H/M-O use |
